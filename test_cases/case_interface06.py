...
def get_excel_info():
    if isinstance(interface_init.initial,Initialization)!=True:
        Init()
    execExcel = ExecExcel()
    list_excel_info= execExcel.get_info_ddt('interface06')
    return  list_excel_info


@ddt
class CaseInterface06(InterFaceCase):

    @classmethod
    def setUpClass(cls):

        interface_init.initial.result=[]
        cls.interfaceId='interface06'

    def setUp(self):
        #接口ID，标识唯一接口信息
        self.interfaceId='interface06'
    ...
    @data(get_excel_info()[1],get_excel_info()[2],get_excel_info()[3],get_excel_info()[4],
          ...
          get_excel_info()[77], get_excel_info()[78])
    def test_interface06(self,excel_info):
        ...
        #接口用例为N则不执行直接成功
        if excel_info['exec'] in ['N','n']:
            self.logger.info('CaseTest.test_interface06|data drivern:skip this use case!')
            return 'success'
        ...
        #写日志，并且根据response值做断言
        self.logger.info('Run case:CaseTest.test_interface06|data drivern')
        try:
            self.assertEqual(str(response.json().get('code')),assert_code)
            interface_init.initial.result.append((case_index,'success','OK'))
        #将错误异常捕获并抛出
            ...
            interface_init.initial.result.append((case_index,'Fail',str(e)))
            self.logger.info("Run case:CaseTest.test_interface06|data drivern error: %s" %str(e))
