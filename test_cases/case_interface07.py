...
@ddt
class CaseInterface07(InterFaceCase):

    @classmethod
    def setUpClass(cls):

        interface_init.initial.result=[]
        cls.interfaceId='interface07'

    def setUp(self):
        #接口ID，标识唯一接口信息
        self.interfaceId='interface07'
    ...
    def test_interface07(self,excel_info):
        ...
        #接口用例为N则不执行直接成功
        if excel_info['exec'] in ['N','n']:
            self.logger.info('CaseTest.test_interface07|data drivern:skip this use case!')
            return 'success'
        ...
        #写日志，并且根据response值做断言
        self.logger.info('Run case:CaseTest.test_interface07|data drivern')
        try:
            self.assertEqual(str(response.json().get('code')),assert_code)
            interface_init.initial.result.append((case_index,'success','OK'))
        #将错误异常捕获并抛出
            ...
            interface_init.initial.result.append((case_index,'Fail',str(e)))
            self.logger.info("Run case:CaseTest.test_interface07|data drivern error: %s" %str(e))
            raise e
