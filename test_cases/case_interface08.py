...
class CaseInterface08(InterFaceCase):
    ...
    def test_interface08(self,excel_info):
        ...
        #接口用例为N则不执行直接成功
        if excel_info['exec'] in ['N','n']:
            self.logger.info('CaseTest.test_interface08|data drivern:skip this use case!')
            return 'success'
        ...
        #将错误异常捕获并抛出
            ...
            interface_init.initial.result.append((case_index,'Fail',str(e)))
            self.logger.info("Run case:CaseTest.test_interface08|data drivern error: %s" %str(e))
            raise e
